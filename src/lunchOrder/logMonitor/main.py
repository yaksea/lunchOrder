...
import tornado
import os
from lunchOrder.logMonitor.handlers import default, HANDLER_ROOT
    ...
    for item in os.listdir(curDir):
        ...
        if os.path.isfile(itemPath):
            ...
            if info[1] == '.py' and not info[0].startswith('_'):
                ...
                moduleName = ('lunchOrder.logMonitor.handlers.'+innerPath).replace('/','.')
...
def run():
    ...
    application = tornado.web.Application(getHandlers(), 
                                      debug=True, **_settings)
    application.listen(8071)
