...
'''
from lunchOrder.base.handlers.formHandler import FormRequestHandler
from lunchOrder.folunchOrder.formAdapter import getFieldOption
from lunchOrder.data.mongodbManager import mongo
from lunchOrder.data.sys import getCategoryDict
from lunchOrder.base.handlers.jsonHandler import JsonRequestHandler
from lunchOrder.common import utility
from lunchOrder.form import formAdapter
import decimal
import datetime
import pymongo
from lunchOrder.base.handlers.dataHandler import DataRequestHandler
    ...
    def getList(self):
        ...
        for item in cursor:
            ...
            if handler in ('Create', 'QuickCreate','BatchCreate','TaobaoImport','Import'):
                ...
                if id:
                    name = mongo.user.find_one({'_id':id['_id']}, {'realName':1})['realName']
                ...
                if ids:
                    ...
                    realNames = []
                    for user in mongo.user.find({'_id':{'$in':ids}}, {'realName':1}):
                        realNames.append(user['realName'])
    
                    des.append('%s数据共享人: [%s]'%(action, ', '.join(realNames)))
            ...
            if des:
                di['descriptions'] = des
                di['realName'] = item['identity']['realName']
                results.append(di)
